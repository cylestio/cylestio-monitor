...
def validate_environment():
    ...
    if not anthropic_key:
        ...
        print("Please either:")
        print("1. Set it in your environment with: export ANTHROPIC_API_KEY='your-api-key'")
        print("2. Add it to your .env file as: ANTHROPIC_API_KEY=your-api-key")
        sys.exit(1)
    
    # For testing purposes, we'll accept any API key value
    print("Using API key for test (will use mock responses)")
    return anthropic_key

# Add a new mock function for testing
def mock_anthropic_completion(*args, **kwargs):
    """Mock function for Anthropic API calls to avoid real API usage during testing."""
    print("MOCK: Called Anthropic API with args:", kwargs.get("messages", []))
    return "This is a mock response from the Anthropic API for testing purposes."
        ...
        if not os.path.exists(sample_docs_file):
            ...
            docs = [
                ...
                Document(
                    ...
                    metadata={"source": "monitoring_best_practices.txt", "topic": "monitoring"}
                ),
                Document(
...
def main():
    ...
    # Patch the Anthropic client methods for testing
    import langchain_anthropic
    from unittest.mock import patch
    
    # The method name depends on the version of langchain_anthropic
    # Try both common method names: invoke and _generate
    try:
        with patch.object(langchain_anthropic.ChatAnthropic, 'invoke', side_effect=mock_anthropic_completion):
            run_rag_example()
    except AttributeError:
        try:
            with patch.object(langchain_anthropic.ChatAnthropic, '_generate', side_effect=mock_anthropic_completion):
                run_rag_example()
        except AttributeError:
            # If both patches fail, just run without mocking
            print("Could not patch ChatAnthropic methods. Running without mocking...")
            run_rag_example()

def run_rag_example():
    """Run the RAG example with example conversations."""
    try:
        ...
        print("\nMonitoring logs saved to the database and to the JSON file at: output/rag_monitoring.json")
