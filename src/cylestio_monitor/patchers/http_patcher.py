...
SUSPICIOUS_SQL_PATTERNS = [
    ...
    r"WHERE\s+\w+\s*=\s*['\"].*[|;&`].*['\"]",  # Typical command injection
    # Mode switching attempts (generic, not specific to the example)
    r"SWITCH.*MODE",
    r"ENABLE.*SHELL",
    r"SET.*MODE",
    # Suspicious query structure (WHERE clause with command-like content)
    r"WHERE\s+\w+\s*=\s*['\"](cat|ls|pwd|whoami|id|echo|curl|wget)['\"]",
    # Additional patterns for MCP-style RCE detection
    r"WHERE\s+\w+\s*=\s*['\"].*-.*['\"]",  # Command with options (like ls -la)
    r"WHERE\s+\w+\s*=\s*['\"].*(\/bin\/|\/usr\/bin\/|cmd\.exe|powershell\.exe).*['\"]"  # Direct path references
]

# MCP-specific patterns for extracting commands from SQL queries
MCP_COMMAND_EXTRACTION_PATTERNS = [
    r"WHERE\s+\w+\s*=\s*['\"](.*?)['\"]",  # WHERE name = 'command'
    r"WHERE\s+\w+\s*=\s*[\"'](.*?)['\"]\s+",  # More permissive pattern
    r"WHERE\s+\w+\s*=\s*[\"](.*?)[\"]",  # Double quotes specifically
    r"WHERE\s+\w+\s*LIKE\s*['\"](.*?)['\"]"  # WHERE name LIKE 'command'
]
            ...
            if "messages" in data and isinstance(data["messages"], list):
                for msg in data["messages"]:
                    if isinstance(msg, dict) and "content" in msg and isinstance(msg["content"], str):
                        sql_queries.append(msg["content"])
        ...
        for match in matches:
            if len(match.groups()) > 0:
                ...
                shell_commands = [
                    ...
                    "curl", "wget", "chmod", "mkdir", "rm", "cp", "mv", "find", "grep",
                    "ping", "telnet", "nc", "ncat", "ssh", "scp"
                ]
                
                # Check if the command starts with any known shell command
                is_shell_command = False
                for cmd in shell_commands:
                    if command.startswith(cmd) or command.startswith(f"./{cmd}") or command.startswith(f"/bin/{cmd}"):
                        is_shell_command = True
                        break
                
                # Also check for path-like structures or command options
                if (not is_shell_command and
...
def _register_virtual_shell_execution(command: str, url: str, method: str) -> None:
    ...
    # Generate an immediate alert, since this is a high-confidence detection
    alert_attributes = {
