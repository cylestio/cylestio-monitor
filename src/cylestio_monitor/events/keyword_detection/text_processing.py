...
try:
    ...
    logger.warning(
        f"Error loading suspicious keywords from config: {e}, using defaults"
    )
    suspicious_keywords = DEFAULT_SUSPICIOUS_KEYWORDS
    ...
    for keyword in suspicious_keywords:
        if len(keyword.split()) == 1:  # Only apply to single words
            ...
            if re.search(pattern, normalized):
                logger.info(
                    f"Suspicious keyword found (word boundary): '{keyword}' in '{normalized}'"
                )
                return True
...
def contains_dangerous(text: str) -> bool:
    ...
    for cmd_fragment in ["drop table", "rm -rf", "exec("]:
        if cmd_fragment in normalized:
            logger.info(
                f"Dangerous command fragment found: '{cmd_fragment}' in '{normalized}'"
            )
            return True
